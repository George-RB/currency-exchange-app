...
router.post(
  'https://currency-exchange-app-zkkc.onrender.com/exchange',
  simpleAuth,
  async (req, res) => {
    ...
    try {
      ...
      if (!amount || amount <= 0) {
        ...
        return res
          .status(400)
          .json({ error: 'Сумма должна быть положительной' });
      }
      ...
      connection.query(
        ...
        [
          user.login,
          `Обмен ${amount} ${fromCurrency} -> ${result.toFixed(
            2
          )} ${toCurrency}`,
        ],
        (error) => {
          if (error) {
            ...
            return res
              .status(500)
              .json({ error: 'Ошибка сохранения операции' });
          }
...
router.get(
  'https://currency-exchange-app-zkkc.onrender.com/history',
  simpleAuth,
  (req, res) => {
    const user = req.user;
